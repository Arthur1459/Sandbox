...
def KeepInSquare(coord, dimension=vr.rect_size, delta=0):
    ...
    x_m, y_m = vr.middle
    if x_m - w + delta <= coord[0] <= x_m + w - delta:
        if y_m - h + delta <= coord[1] <= y_m + h - delta:
            return coord, False
        elif coord[1] < y_m - h + delta:
            return (coord[0], y_m - h + delta), True
        else:
            return (coord[0], y_m + h - delta), True
    elif coord[0] < x_m - w + delta:
        if y_m - h + delta <= coord[1] <= y_m + h - delta:
            return (x_m - w + delta, coord[1]), True
        elif coord[1] < y_m - h + delta:
            return (x_m - w + delta, y_m - h + delta), True
        else:
            return (x_m - w + delta, y_m + h - delta), True
    else:
        if y_m - h + delta <= coord[1] <= y_m + h - delta:
...
def target_distance(pt):
    return abs(cf.target - pt[0])

def getBestGen(gens):
    best_score, best_pulse, best_phase, best_amp = None, None, None, None
    for gen in gens:
        if best_score is None or (best_score is not None and 0 <= gen[0] < best_score):
            best_score = gen[0]
            best_pulse = gen[1]
            best_phase = gen[2]
            best_amp = gen[3]
    if best_score is None:
        ...
        vr.best_gen_archive.append((best_score, best_pulse, best_phase, best_amp))
        if vr.best_score is None or best_score < vr.best_score:
            vr.best_gen = (best_pulse, best_phase, best_amp)
            vr.best_score = best_score
    return vr.best_gen

def getMutation(gen: list[float],  delta=1):
    new_gen = []
    for gen_part in gen:
        part = []
        for value in gen_part:
            part.append(value + delta * (random() - random()))
        new_gen.append(part)
    return new_gen

def CreateBody(pulse, phase, amps, color=None):
    new_body = Body(pulsation=pulse, phase=phase, amplitudes=amps, color=color)
    for other_body in vr.bodies:
        new_body.add_ignore_ids(other_body.ids)
        other_body.add_ignore_ids(new_body.ids)
    vr.bodies.append(new_body)
    return
